...
class ControlModel(nn.Module):
    ...
    def forward(self, x):
        ...
        q4 = normalize(th.tanh(self.net(th.cat((x, s3, s2), dim=1)).view(batch, 4)))
        qa = hamilton_product(q4, qa)
        s4 = self.skyview(qa).view(batch, 1, 512, 512)

        q5 = normalize(th.tanh(self.net(th.cat((x, s4, s3), dim=1)).view(batch, 4)))
        qa = hamilton_product(q5, qa)
        s5 = self.skyview(qa).view(batch, 1, 512, 512)

        q6 = normalize(th.tanh(self.net(th.cat((x, s5, s4), dim=1)).view(batch, 4)))
